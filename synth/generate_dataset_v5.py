"""
BoneScintiVision — YOLOデータセット v5（腹部病変オーバーサンプリング最強化版）

EXP-006: 腹部Recall 0.757 → 0.800 達成を目指す。

EXP-005の課題:
  - 腹部Recall=0.757（目標0.800未達）
  - 腹部病変45%オーバーサンプリングは効果あり（0.724→0.757、+3.3pp）
    だが十分でない

v5 の差分 (v4比):
  - 生理的集積なし比率: **50% → 60%** (さらにFP抑制)
  - 腹部病変強制追加確率: **45% → 65%** (腹部Recall強化)
  - 生成数: **3540 → 5000枚** (データ量増加で汎化性向上)
  - シード: 42 (v4=2026)
  - 出力先: data/yolo_dataset_v5

使い方:
  python3.12 synth/generate_dataset_v5.py
  python3.12 synth/generate_dataset_v5.py --n 5000 --out data/yolo_dataset_v5
"""
...
N_WORKERS = 8
CLASS_HOT_SPOT = 0
OUT_DIR = BASE_DIR / "data" / "yolo_dataset_v5"

# v5: 生理的集積なし比率を60%に引き上げ（v4=50%）
NO_PHYSIO_PROB = 0.60
...
def _sample_lesions_v5(phantom: BonePhantom, n_lesions: int, rng: np.random.Generator) -> List[Dict]:
    """
    v5用病変サンプリング: 腹部病変オーバーサンプリング最強化（65%）。
    v4比: 腹部強制追加 45% → 65%
    """
    lesions = phantom.sample_lesion_sites(n_lesions)

    # 65%の確率で腹部/骨盤病変を1〜2個強制追加（v4=45%）
    if rng.random() < 0.65 and n_lesions > 0:
...
def generate_one_v5(args_tuple) -> Tuple[str, str, int]:
    """前後面デュアルビュー1枚 + YOLOラベル生成 (v5)."""
    ...
    lesions = _sample_lesions_v5(phantom, n_lesions, rng) if n_lesions > 0 else []
    ...
    fname = f"bone_scan_v5_{split}_{idx:05d}"
...
def generate_dataset_v5(
    ...
    t0 = time.time()
    rng_main = np.random.default_rng(42)  # v5用シード
    seeds = rng_main.integers(0, 1_000_000, size=n_total).tolist()

    print("=" * 60)
    print("  BoneScintiVision v5 — 腹部病変オーバーサンプリング最強化版")
    print(f"  Train: {n_train} / Val: {n_val}")
    print(f"  出力先: {out_dir}")
    print(f"  変更点: 生理的集積なし 50%→60%, 腹部オーバーサンプリング 45%→65%, 5000枚")
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        futures = {executor.submit(generate_one_v5, t): t for t in tasks}
    ...
    with open(yaml_path, "w") as f:
        f.write(f"""# BoneScintiVision v5 — 腹部病変オーバーサンプリング最強化版データセット
...
# v5変更点: 生理的集積なし 50%→60%, 腹部オーバーサンプリング 45%→65%, 5000枚
# EXP-006: 腹部Recall 0.757→0.800 達成目標
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="BoneScintiVision v5 dataset (腹部強化+5000枚)")
    ...
    generate_dataset_v5(args.n, args.val_ratio, Path(args.out), args.workers)
