"""
BoneScintiVision — YOLOデータセット v6（大規模 + 腹部強化版）

EXP-006: データ量 3540→6000枚 + 腹部病変強制追加 45%→60% で腹部Recall≥0.800を狙う。

EXP-005の課題:
  - 腹部Recall=0.757 (目標0.800未達, v4比+3.3pp)
  - データ量3540枚では腹部病変の学習が不十分

v6 の差分 (v4比):
  - **生成数: 3540 → 6000枚** (train=5100 / val=900)
  - **腹部病変強制追加: 45% → 60%** (腹部ラベル密度向上)
  - 生理的集積なし比率: 50% (v4と同)
  - シード: 2025 (v4=2026)
  - 出力先: data/yolo_dataset_v6

使い方:
  python3.12 synth/generate_dataset_v6.py
  python3.12 synth/generate_dataset_v6.py --n 6000 --out data/yolo_dataset_v6
"""
...
DEFAULT_N_TOTAL = 6000
DEFAULT_VAL_RATIO = 0.15
ANT_W = POST_W = IMG_H = 256
FULL_W = ANT_W + POST_W
N_WORKERS = 10
CLASS_HOT_SPOT = 0
OUT_DIR = BASE_DIR / "data" / "yolo_dataset_v6"

# v6: 生理的集積なし比率50%維持 (v4と同じ)
NO_PHYSIO_PROB = 0.50
# v6: 腹部病変強制追加確率を45%→60%に引き上げ
ABDOMEN_OVERSAMPLE_PROB = 0.60
...
def generate_one_v6(args_tuple) -> Tuple[str, str, int]:
    ...
    lesions = _sample_lesions_v6(phantom, n_lesions, rng) if n_lesions > 0 else []
    ...
    fname = f"bone_scan_v6_{split}_{idx:05d}"
...
def generate_dataset_v6(
    ...
    print("=" * 60)
    print("  BoneScintiVision v6 — 生理的集積抑制強化版")
    print(f"  Train: {n_train} / Val: {n_val}")
    print(f"  出力先: {out_dir}")
    print(f"  変更点: データ量3540→6000, 腹部60%強制追加, 生理的集積なし50%")
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        futures = {executor.submit(generate_one_v6, t): t for t in tasks}
    ...
    with open(yaml_path, "w") as f:
        f.write(f"""# BoneScintiVision v6 — 生理的集積抑制強化版データセット
...
if __name__ == "__main__":
    ...
    generate_dataset_v6(args.n, args.val_ratio, Path(args.out), args.workers)
