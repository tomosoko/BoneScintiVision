"""
BoneScintiVision — YOLOデータセット v3（拡張版デュアルビュー）

EXP-003: 腹部骨盤Recall改善を目的とした拡張データセット。

v2 との差分:
  - 生成数: 3540枚 (train 3000 / val 540)  ← v2の1.7倍
  - 生理的集積バリエーション: 0〜100%強度範囲でランダム化
    (30%の確率で腎臓・膀胱なし → モデルが生理的uptakeに依存しないよう学習)
  - 腹部病変オーバーサンプリング: 30%確率で腹部/骨盤病変を強制追加
  - 小病変（径6〜10px）の比率増加: 10% → 20%

使い方:
  python3.12 synth/generate_dataset_v3.py
  python3.12 synth/generate_dataset_v3.py --n 3540 --out data/yolo_dataset_v3
"""
...
N_WORKERS = 8
CLASS_HOT_SPOT = 0
OUT_DIR = BASE_DIR / "data" / "yolo_dataset_v3"

# 腹部/骨盤に含まれる部位（Y > 55% 相当）
_ABDOMEN_REGIONS = {"lumbar", "pelvis", "sacrum", "femur_l", "femur_r"}


def _sample_lesions_v3(phantom: BonePhantom, n_lesions: int, rng: np.random.Generator) -> List[Dict]:
    """
    v3用病変サンプリング: 腹部病変オーバーサンプリング + 小病変比率増加。
    """
    lesions = phantom.sample_lesion_sites(n_lesions)

    # 30%の確率で腹部/骨盤病変を1〜2個強制追加
    if rng.random() < 0.30 and n_lesions > 0:
...
def generate_one_v3(args_tuple) -> Tuple[str, str, int]:
    ...
    lesions = _sample_lesions_v3(phantom, n_lesions, rng) if n_lesions > 0 else []
    ...
    # 生理的集積: 30%確率でなし、70%確率であり（強度ランダム）
    add_phys = rng.random() > 0.30
    ...
    fname = f"bone_scan_v3_{split}_{idx:05d}"
...
def generate_dataset_v3(
    ...
    t0 = time.time()
    rng_main = np.random.default_rng(43)  # v3用シード
    seeds = rng_main.integers(0, 1_000_000, size=n_total).tolist()

    print("=" * 60)
    print("  BoneScintiVision v3 — 拡張デュアルビューデータセット生成")
    print(f"  Train: {n_train} / Val: {n_val}")
    print(f"  出力先: {out_dir}")
    print(f"  新機能: 腹部病変強化+生理的集積ランダム化")
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        futures = {executor.submit(generate_one_v3, t): t for t in tasks}
    ...
    with open(yaml_path, "w") as f:
        f.write(f"""# BoneScintiVision v3 — 拡張デュアルビュー hot spot 検出データセット
...
# v3拡張: 腹部病変オーバーサンプリング、生理的集積ランダム化
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="BoneScintiVision v3 dataset")
    ...
    generate_dataset_v3(args.n, args.val_ratio, Path(args.out), args.workers)
