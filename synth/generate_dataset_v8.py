"""
BoneScintiVision — YOLOデータセット v8（生理的集積比率調整版）

EXP-009: 生理的集積なし比率を70%→55%に下げて再訓練。
EXP-007/008の課題:
  - EXP-007: 腹部Recall=0.828達成(目標達成)、しかしP=0.707(FP過多)
  - EXP-008: physio mask後処理でもP=0.718止まり(目標P≥0.900未達)
  - 原因仮説: no_physio=70%が高すぎ→モデルが生理的集積を学習不足→FP増加

v8 の差分 (v7比):
  - **生理的集積なし比率: 70% → 55%** (生理的集積を学習させて FP 抑制)
  - 腹部病変強制追加: 80%（v7と同一、腹部Recall維持）
  - 生成数: 7000枚 (v7と同一)
  - シード: 2023 (v7=2024)
  - 出力先: data/yolo_dataset_v8

使い方:
  python3.12 synth/generate_dataset_v8.py
  python3.12 synth/generate_dataset_v8.py --n 7000 --out data/yolo_dataset_v8
"""
...
N_WORKERS = 10
CLASS_HOT_SPOT = 0
OUT_DIR = BASE_DIR / "data" / "yolo_dataset_v8"

# v8: 生理的集積なし比率を55%に引き下げ（v7=70%）
# 狙い: モデルが生理的集積パターンを学習→腎臓・膀胱周辺のFP削減
NO_PHYSIO_PROB = 0.55
# v8: 腹部病変強制追加確率はv7と同一（腹部Recall維持）
...
def _sample_lesions_v8(phantom: BonePhantom, n_lesions: int, rng: np.random.Generator) -> List[Dict]:
    """
    v8用病変サンプリング: 腹部病変オーバーサンプリング80%（v7と同一）。
    """
    lesions = phantom.sample_lesion_sites(n_lesions)

    # 80%の確率で腹部/骨盤病変を1〜2個強制追加
...
def generate_one_v8(args_tuple) -> Tuple[str, str, int]:
    """前後面デュアルビュー1枚 + YOLOラベル生成 (v8)."""
    ...
    lesions = _sample_lesions_v8(phantom, n_lesions, rng) if n_lesions > 0 else []
    ...
    # 生理的集積: 55%確率でなし（v7=70%→v8=55%、生理的集積学習強化）
    ...
    fname = f"bone_scan_v8_{split}_{idx:05d}"
...
def generate_dataset_v8(
    ...
    t0 = time.time()
    rng_main = np.random.default_rng(2023)  # v8用シード（v7=2024, v6=2025）
    seeds = rng_main.integers(0, 1_000_000, size=n_total).tolist()

    print("=" * 60)
    print("  BoneScintiVision v8 — 生理的集積比率調整版")
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        futures = {executor.submit(generate_one_v8, t): t for t in tasks}
    ...
    with open(yaml_path, "w") as f:
        f.write(f"""# BoneScintiVision v8 — 生理的集積比率調整版データセット
...
# v8変更点: 生理的集積なし比率70%→55%（生理的集積学習強化でFP抑制）
# EXP-009: Precision改善（EXP-007 P=0.707→目標P≥0.900）while 腹部Recall≥0.800維持
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="BoneScintiVision v8 dataset (生理的集積比率調整版)")
    ...
    generate_dataset_v8(args.n, args.val_ratio, Path(args.out), args.workers)
