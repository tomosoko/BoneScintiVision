...
- **EXP-008: 生理的集積マスク後処理 実装完了**（未実行）
  - `models/physio_mask.py`: 腎臓・膀胱の除外ゾーン + `filter_physio_detections()`
  - `models/validate_detector_v8.py`: suppress_conf スイープ評価
  - 次: `python3.12 models/validate_detector_v8.py` で効果測定
  - 改善不十分なら: 生理的集積なし比率調整（v7: 70% → v8: 55%）+ 再訓練
