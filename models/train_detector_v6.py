"""
BoneScintiVision — EXP-006 訓練スクリプト (YOLO11m, v6データ, 6000枚+腹部60%)

EXP-005の課題:
  - 腹部Recall=0.757（目標0.800未達）

EXP-006の戦略:
  - データ: yolo_dataset_v6（6000枚, 腹部病変60%, 生理的集積なし50%）
  - モデル・ハイパーパラメータはEXP-005と同一（変数を一つだけ変える）
  - 期待: データ量増加 + 腹部オーバーサンプリング強化で腹部Recall≥0.800

v5 との差分:
  - データ: yolo_dataset_v5(5000枚,腹部65%) → yolo_dataset_v6(6000枚,腹部60%)
  - その他: 同一

使い方:
  python3.12 synth/generate_dataset_v6.py     # データ生成（先に実行）
...
DATA_YAML = BASE_DIR / "data" / "yolo_dataset_v6" / "dataset.yaml"
...
def main():
    ...
    if not DATA_YAML.exists():
        ...
        print("先に python3.12 synth/generate_dataset_v6.py を実行してください。")
        return
