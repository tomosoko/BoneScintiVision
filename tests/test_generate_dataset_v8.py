"""generate_dataset_v8.py のユニットテスト."""
import sys
import os
import numpy as np
...
class TestGenerateOneV8:
    """generate_one_v8 の基本テスト."""

    def _gen(self, tmpdir, idx=0, split="train"):
        from synth.generate_dataset_v8 import generate_one_v8
        img_dir = Path(tmpdir) / split / "images"
        lbl_dir = Path(tmpdir) / split / "labels"
        img_dir.mkdir(parents=True, exist_ok=True)
        lbl_dir.mkdir(parents=True, exist_ok=True)
        result = generate_one_v8((idx, split, str(img_dir), str(lbl_dir), idx * 7))
    ...
    def test_filename_has_v8_prefix(self, tmp_path):
        """v8ファイル名プレフィックス確認."""
        (img_path, _, _), _, _ = self._gen(tmp_path)
        assert "bone_scan_v8" in Path(img_path).name
...
class TestNoPHYSIOProbV8:
    """v8: 生理的集積なし確率が55%に設定されていることを確認."""

    def test_no_physio_prob_is_55_percent(self):
        from synth.generate_dataset_v8 import NO_PHYSIO_PROB
        assert abs(NO_PHYSIO_PROB - 0.55) < 1e-9

    def test_no_physio_prob_lower_than_v7(self):
        """v8のNO_PHYSIO_PROBはv7(70%)より低い（生理的集積学習強化）."""
        ...
        assert v8_prob < v7_prob

    def test_abdomen_oversample_prob_is_80_percent(self):
        """v8: 腹部オーバーサンプリング確率が80%（v7と同一）."""
        from synth.generate_dataset_v8 import ABDOMEN_OVERSAMPLE_PROB
        assert abs(ABDOMEN_OVERSAMPLE_PROB - 0.80) < 1e-9

    def test_abdomen_oversample_same_as_v7(self):
        """v8のABDOMEN_OVERSAMPLE_PROBはv7と同一（腹部Recall維持）."""
        from synth.generate_dataset_v8 import ABDOMEN_OVERSAMPLE_PROB as v8_prob
        from synth.generate_dataset_v7 import ABDOMEN_OVERSAMPLE_PROB as v7_prob
        assert abs(v8_prob - v7_prob) < 1e-9


class TestSampleLesionsV8:
    """v8: _sample_lesions_v8 の動作テスト."""

    def test_returns_list(self):
        from synth.generate_dataset_v8 import _sample_lesions_v8
        from synth.bone_phantom import BonePhantom
        phantom = BonePhantom(seed=42)
        rng = np.random.default_rng(42)
        result = _sample_lesions_v8(phantom, 3, rng)
    ...
    def test_zero_lesions_returns_empty(self):
        ...
        phantom = BonePhantom(seed=42)
        rng = np.random.default_rng(42)
        result = _sample_lesions_v8(phantom, 0, rng)
        assert result == []
    ...
    def test_abdomen_oversample_rate_near_80_percent(self):
        ...
        # v8は80%確率で腹部追加 → 55〜95%の範囲で追加されるはず
    ...
    def test_lesion_coordinates_within_image_bounds(self):
        ...
        for seed in range(20):
            ...
            result = _sample_lesions_v8(phantom, 5, rng)
            for les in result:
