...
### 結果
- 実行コマンド: `python3.12 models/validate_detector_v8.py`
- 結果は実行後にここへ追記予定

### 考察（実行前推定）
- 生理的集積ゾーン内 FP が減少 → Precision 改善が期待される
- 高信頼度の骨転移検出は保持されるため Recall への影響は限定的
- ただし合成データでは骨転移と腎臓の重なりが少なく、改善幅は限定的な可能性あり
- **根本解決は次の2択**: (a) 生理的集積なし比率調整 (b) 2 段階検出で腹部特化
