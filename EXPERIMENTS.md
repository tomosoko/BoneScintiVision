...
### 結果（200枚評価, conf=0.25, IoU>0.3）

| 指標 | EXP-005 | EXP-003b | 差分 |
|---|---|---|---|
| Precision | **0.980** ✅ | 0.965 | +1.5pp ↑ |
| Recall | **0.818** ✅ | 0.808 | +1.0pp ↑ |
| F1 | **0.892** | 0.882 | +1.0pp ↑ |
| mAP50 (YOLOval) | **0.864** | 0.872 | -0.8pp |
| 病変数MAE | 0.69 | 0.69 | 同等 |
...
**考察:**
- Precision 0.965 → 0.980 (+1.5pp): 生理的集積なし50%が功を奏し、FP削減成功 ✅
- 全体 Recall 0.808 → 0.818 (+1.0pp): 目標達成 ✅
- **腹部Recall: 0.724 → 0.757 (+3.3pp)** 改善したが目標0.800は未達 ❌
  - 根本原因: 腎臓・膀胱の生理的集積と腹部病変の識別が依然困難
  - 腹部オーバーサンプリング45%は効果あるが十分でない
- **結論**: EXP-005は全体的に改善。腹部Recall≥0.800は次実験の課題

**次の方針 (EXP-006案):**
1. 腹部オーバーサンプリング: 45% → 65%
2. 生理的集積なし: 50% → 60%（さらにFP抑制）
3. または imgsz=640 で細部解像度向上
